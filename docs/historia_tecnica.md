...
| BELCORP_ANALYTICS.COMERCIAL.MARCAS | Marcas Belcorp | MARCA_ID, NOMBRE_MARCA (Esika, L'Bel, Cyzone) |
...
Se revisaron 60 lineas del archivo `sql/analisis_cumplimiento.sql` y se detectaron 7 errores:
...
| 3 | `LIMIT 3` - la historia pide 6 campanas | Cambiado a `LIMIT 6` | 20 |
...
| 7 | `JOIN ventas_reales` excluye metas sin ventas | Cambiado a `LEFT JOIN` para incluir cumplimiento 0% | 56 |
