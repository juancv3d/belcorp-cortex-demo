...
SELECT
    ...
    CASE
        ...
        WHEN COALESCE(vr.VENTA_REAL, 0) >= mc.VENTA_PLANIFICADA * 0.9 THEN 'Cerca de Meta'
        ELSE 'Bajo'
...
JOIN ventas_reales vr ON mc.CAMPANA_ID = vr.CAMPANA_ID
    AND mc.MARCA_ID = vr.MARCA_ID
    AND mc.CATEGORIA_ID = vr.CATEGORIA_ID
ORDER BY c.PAIS, c.NUMERO_CAMPANA, m.NOMBRE, cat.NOMBRE_CATEGORIA;
